...
if [[ -n "$EXISTS" ]]; then
    ...
    docker exec -i "$CONTAINER" \
    psql -U postgres -d "$DB_NAME" -c "CREATE TABLE $TABLE_NAME ( id SERIAL PRIMARY KEY, name TEXT, email TEXT, age INT ); INSERT INTO users (name, email, age) VALUES ('Alice', 'alice@test.com', 24), ('Bob', 'bob@test.com', 42), ('Alex', 'alex@test.com', 31); SELECT * FROM users;"
